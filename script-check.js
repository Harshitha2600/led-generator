...
function renderTextCanvas(canvas, ctx) {
  ...
  const lineHeight = canvas.height / active.length;
...
function drawImageNormal(img, canvas, ctx) {
  ctx.fillStyle = 'black';
  ctx.fillRect(0, 0, canvas.width, canvas.height);
...
function renderImagePreview(canvas, ctx, source) {
  const img = new Image();
...
function renderPreview() {
  ...
  // Always render on solid black background
  ctx.fillStyle = 'black';
  ctx.fillRect(0, 0, canvas.width, canvas.height);
  ...
  if (contentType === 'image') {
    ...
    renderImagePreview(canvas, ctx, imageFile);
    return;
  }
  ...
  if (contentType === 'url') {
    ...
    renderImagePreview(canvas, ctx, imageURL);
    return;
  }

  // Text rendering
  renderTextCanvas(canvas, ctx);
}
